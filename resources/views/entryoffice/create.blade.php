<x-app-layout>
                        ...
                        <div class="patient_information">
                            ...
                            <div class="patient_information_wrapper justify-content-between" style="display: none">
                                ...
                                <div class="pi_right">
                                    ...
                                    <div style="display: flex; gap:1rem; border-bottom:1px solid #cccccc66;">
                                        <div class="pi_right_block" style="border:none; width:40%; margin-bottom: 0;">
                                            <strong>Numéro de l'employeur</strong>
                                            <span class="matricule"></span>
                                        </div>
                                        <div class="pi_right_block"
                                            style="border:none; padding-left:1rem; border-left:1px solid #ddd; margin-bottom: 0;">
                                    ...
                                    <div class="pi_right_block">
                                        <button class="btn btn-primary-light btn-wave print_family_file"
                                            style="display: none;">Imprimer la fiche
                                            familiale</button>
                                    </div>
                                </div>
                            </div>
    ...
    @section('js')
        ...
        <script>
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.status) {
                                ...
                                const canPrintFamily = response.canprintfamily;
                                // console.log(canPrint);
                                let html = '';

                                // Show or hide the print_family_file button based on permission
                                if (canPrintFamily) {
                                    $('.print_family_file').show();
                                } else {
                                    $('.print_family_file').hide();
                                }
                                ...
                                // Only show the family print button when canPrintFamily equals exactly 1
                                if (canPrintFamily == 1) {
                                    $('.print_family_file').show();
                                } else {
                                    $('.print_family_file').hide();
                                ...
                                patients.forEach(function(patient) {
                                    if (patient.RelationCode == 1) {
                                        ...
                                        $('.matricule_assure').text(
                                            `${patient['Compe Assuré']}`);
                                        $('.ssn').text(
                                            `${patient.SSN}`);
                                        $('.regime').text(
                                            `${patient['Regime Travailleur']}`);
                                        $('.regime_amo_pass').text(
                                            `${patient['Regime']}`);
            ...
            $(document).on('click', '.print_family_file', function(e) {
                e.preventDefault();
                const ssn = $('#ssn').val();
                ...
                if (!ssn) {
                    Swal.fire({
                        icon: 'error',
                        title: 'Erreur',
                        text: 'Veuillez d\'abord rechercher un patient',
                    });
                    return;
                }

                // Use the SSN directly without storing data
                printExternal("{{ route('printing.familyform', '') }}" + '/' + ssn);
            });
        </script>
